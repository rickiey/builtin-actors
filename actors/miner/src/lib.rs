...
impl Actor {
        ...
        RT: Runtime<BS>,
    {
        ...
        if !matches!(new_address.protocol(), Protocol::ID) {
            return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "owner address must be an ID address"));
        }
        ...
        RT: Runtime<BS>,
    {
        ...
        let unlocked_balance = state.get_unlocked_balance(&rt.current_balance()).map_err(|_e| {
            actor_error!(USR_ILLEGAL_STATE, "failed to determine unlocked balance")
        })?;
        if unlocked_balance < aggregate_fee {
            return Err(actor_error!(
                USR_INSUFFICIENT_FUNDS,
        ...
        RT: Runtime<BS>,
    {
        ...
        for update in params.updates.iter() {
            ...
            if !state
                ...
                .map_err(|_| actor_error!(USR_ILLEGAL_ARGUMENT, "error checking sector health"))?
            {
        ...
        if validated_updates.is_empty() {
            return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "no valid updates"));
        }
        ...
        let succeeded_sectors = rt.transaction(|state: &mut State, rt| {
                ...
                for (i, with_details) in decls_by_deadline[&dl_idx].iter().enumerate() {
                    ...
                    if initial_pledge_at_upgrade > with_details.sector_info.initial_pledge {
                        ...
                        let unlocked_balance = state
                            ...
                            .map_err(|_|
                                actor_error!(USR_ILLEGAL_STATE, "failed to calculate unlocked balance")
                            )?;
                    ...
                    let mut partition = partitions
                        ...
                        .cloned()
                        .ok_or_else(|| actor_error!(USR_NOT_FOUND, "no such deadline {} partition {}", dl_idx, with_details.update.partition))?;
            ...
            BitField::try_from_bits(succeeded).map_err(|_|{
                actor_error!(USR_ILLEGAL_ARGUMENT; "invalid sector number")
            })
        })?;
        ...
        let (pledge_delta, mut to_burn, power_delta, to_reward) =
            rt.transaction(|st: &mut State, rt| {
                ...
                if verify_windowed_post(rt, target_deadline.challenge, &sector_infos, proofs)? {
                    return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "failed to dispute valid post"));
                } else {
        ...
        RT: Runtime<BS>,
    {
        let curr_epoch = rt.curr_epoch();
        {
            ...
            if params.sectors.is_empty() {
                return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "batch empty"));
        ...
        for precommit in params.sectors.iter() {
            ...
            if !is_sealed_sector(&precommit.sealed_cid) {
                return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "sealed CID had wrong prefix"));
            }
            ...
            if precommit.replace_capacity {
                return Err(ActorError::new(
                    SYS_FORBIDDEN,
                    "cc upgrade through precommit discontinued, use ProveReplicaUpdate".into(),
                ));
            }
        ...
        rt.transaction(|state: &mut State, rt| {
            ...
            if consensus_fault_active(&info, curr_epoch) {
                return Err(actor_error!(USR_FORBIDDEN, "pre-commit not allowed during active consensus fault"));
            }
            ...
            for (i, precommit) in params.sectors.iter().enumerate() {
                ...
                if sector_wpost_proof != info.window_post_proof_type {
                    return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "sector Window PoSt proof type %d must match miner Window PoSt proof type {} (seal proof type {})", i64::from(sector_wpost_proof), i64::from(info.window_post_proof_type)));
                }
                if precommit.deal_ids.len() as u64 > deal_count_max {
                    return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "too many deals for sector {} > {}", precommit.deal_ids.len(), deal_count_max));
                }
                ...
                if deal_weight.deal_space > info.sector_size as u64 {
                    return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "deals too large to fit in sector {} > {}", deal_weight.deal_space, info.sector_size));
                }
                if precommit.replace_capacity {
                ...
                .ok_or_else(|| actor_error!(USR_ILLEGAL_ARGUMENT, "no max seal duration set for proof type: {}", i64::from(precommit.seal_proof)))?;
            ...
            if available_balance < total_deposit_required {
                return Err(actor_error!(USR_INSUFFICIENT_FUNDS, "insufficient funds {} for pre-commit deposit: {}", available_balance, total_deposit_required));
            }
        ...
        RT: Runtime<BS>,
    {
        ...
        if params.sector_number > MAX_SECTOR_NUMBER {
            return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "sector number greater than maximum"));
        }
        ...
        let precommit = st
            ...
            .ok_or_else(|| {
                actor_error!(USR_NOT_FOUND, "no pre-commited sector {}", sector_number)
            })?;
        ...
        RT: Runtime<BS>,
    {
        ...
        if params.sector_number > MAX_SECTOR_NUMBER {
            return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "sector number out of range"));
        }
        ...
        match st.get_sector(rt.store(), params.sector_number) {
            ...
            Ok(None) => {
                Err(actor_error!(USR_NOT_FOUND, "sector {} not proven", params.sector_number))
            }
            Ok(Some(_sector)) => Ok(()),
        }
    }
            ...
            for deadline_idx in deadlines_to_load {
                ...
                for decl in &mut decls_by_deadline[deadline_idx as usize] {
                    ...
                    let mut partition = partitions
                        ...
                        .cloned()
                        .ok_or_else(|| {
                            actor_error!(USR_NOT_FOUND, "no such partition {:?}", key)
                        })?;
        ...
        RT: Runtime<BS>,
    {
...

        {
            ...
            to_process
                ...
                .map_err(|e| {
                    actor_error!(USR_ILLEGAL_ARGUMENT, "cannot process requested parameters: {}", e)
                })?;
        }
        ...
        RT: Runtime<BS>,
    {
        ...
        let partitions = params.partitions.validate().map_err(|e| {
            actor_error!(USR_ILLEGAL_ARGUMENT, "failed to parse partitions bitfield: {}", e)
        })?;
        ...
        RT: Runtime<BS>,
    {
        let mask_sector_numbers = params
            .mask_sector_numbers
            .validate()
            .map_err(|e| actor_error!(USR_ILLEGAL_ARGUMENT, "invalid mask bitfield: {}", e))?;

        let last_sector_number = mask_sector_numbers
            .last()
            .ok_or_else(|| actor_error!(USR_ILLEGAL_ARGUMENT, "invalid mask bitfield"))?
            as SectorNumber;
        ...
        RT: Runtime<BS>,
    {
        ...
        let (pledge_delta_total, to_burn) = rt.transaction(|st: &mut State, rt| {
            ...
            let unlocked_balance = st.get_unlocked_balance(&rt.current_balance()).map_err(|e| {
                actor_error!(USR_ILLEGAL_STATE, "failed to calculate unlocked balance: {}", e)
            })?;
            ...
            let newly_vested = st
                ...
                .map_err(|e| {
                    actor_error!(USR_ILLEGAL_STATE, "failed to lock funds in vesting table: {}", e)
                })?;
        ...
        RT: Runtime<BS>,
    {
        ...
        let fault = rt
            ...
            .ok_or_else(|| actor_error!(USR_ILLEGAL_ARGUMENT, "No consensus fault found"))?;
    ...
    RT: Runtime<BS>,
{
    ...
    let max_lifetime = seal_proof_sector_maximum_lifetime(policy, seal_proof, rt.network_version())
        .ok_or_else(|| {
            actor_error!(USR_ILLEGAL_ARGUMENT, "unrecognized seal proof type {:?}", seal_proof)
        })?;
    ...
    RT: Runtime<BS>,
{
    if rt.curr_epoch() <= params.interactive_epoch {
        return Err(actor_error!(USR_FORBIDDEN, "too early to prove sector"));
    }
    ...
    RT: Runtime<BS>,
{
    ...
    if data_commitment_inputs.len() != ret.commds.len() {
        return Err(actor_error!(USR_ILLEGAL_STATE,
    ...
    RT: Runtime<BS>,
{
    let resolved = rt
        .resolve_address(&raw)
        .ok_or_else(|| actor_error!(USR_ILLEGAL_ARGUMENT, "unable to resolve address: {}", raw))?;

    let owner_code = rt
        .get_actor_code_cid(&resolved)
        .ok_or_else(|| actor_error!(USR_ILLEGAL_ARGUMENT, "no code for address: {}", resolved))?;
...
fn check_peer_info(
    ...
    for ma in multiaddrs {
        if ma.0.is_empty() {
            return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "invalid empty multiaddr"));
        }
        total_size += ma.0.len();
    }
    ...
    RT: Runtime<BS>,
{
    ...
    if valid_pre_commits.is_empty() {
        return Err(actor_error!(USR_ILLEGAL_ARGUMENT, "all prove commits failed to validate"));
    }
    ...
    let (total_pledge, newly_vested) = rt.transaction(|state: &mut State, rt| {
        ...
        state.add_pre_commit_deposit(&(-deposit_to_unlock)).map_err(|e| {
            actor_error!(USR_ILLEGAL_STATE, "failed to add precommit deposit: {}", e)
        })?;
        ...
        state
            ...
            .map_err(|e| actor_error!(USR_ILLEGAL_STATE, "failed to add initial pledge: {}", e))?;
